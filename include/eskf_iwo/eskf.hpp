...
// wheel(Vx + Vy + Wz)、imu -> eskf.

#ifndef WHEEL_IMU_ESKF_HPP
#define WHEEL_IMU_ESKF_HPP
...
namespace sad 
{
    /**
     * 书本第 3 章介绍的误差卡尔曼滤波器
     * 可以指定观测 GNSS 的读数，GNSS 应该事先转换到车体坐标系
     *
     * 本书使用 18 维的 ESKF，标量类型可以由 S 指定，默认取 double
     * 变量顺序：p, v, R, bg, ba, grav，与书本对应
     * @tparam S    状态变量的精度，取 float 或 double
     */
    // 指定类型
    ...
    class ESKF
    {
        public:
            ...
            using Vec18T = Eigen::Matrix<S, 18, 1>;         // 18 维向量类型
            using Mat3T = Eigen::Matrix<S, 3, 3>;           // 3x3 矩阵类型
            using MotionNoiseT = Eigen::Matrix<S, 18, 18>;  // 运动噪声类型
            using OdomNoiseT = Eigen::Matrix<S, 3, 3>;      // 里程计噪声类型
            using GnssNoiseT = Eigen::Matrix<S, 6, 6>;      // GNSS 噪声类型
            using Mat18T = Eigen::Matrix<S, 18, 18>;        // 18 维方差类型
    ...
    bool ESKF<S>::Predict(const IMU& imu) 
    {
        ...
        // if(imu.timestamp_ >= current_time_) return false;
        ...
        // TODO: F 实际上是稀疏矩阵，也可以不用矩阵形式进行相乘而是写成散装形式，这里为了教学方便，使用矩阵形式
    ...
    bool ESKF<S>::ObserveWheelSpeed(const Odom& odom) 
    {
        ...
        double average_vel = odom.averageSpeed_;
